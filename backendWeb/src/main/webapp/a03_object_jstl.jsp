...
<%--
# jstl 객체 선언과 사용
1. jstl에서는 기본적으로 mvc에서 넘겨온 vo객체를 el태그형태로
	사용하는 것이 일반적이다.
...
<body>
    <div class="container mt-3">
    	...
    	<%--
    	2. request 범위 Person객체를 session으로 이름 변경해서 할당
			기존에 request 범위의 p01객체가 session 범위로 변경이 됨
    	 --%>
    	<c:set var="person" value="${p01}" scope="session"/>
    	...
    	<%--
    	3. 속성 변경
    		1) 주의 property는 get/set 메서드를 말함
    	...
    	<%--
    	ex) a04_object.jsp
    	Product를 클래스 prod로 request 설정하고
    	session 범위로 prodSess로 처리하여 호출하세요
    	 물건가격 변경 처리
    	 --%>
