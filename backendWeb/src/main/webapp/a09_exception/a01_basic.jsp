...
1. 웹 개발 프로그래밍을 하면 에러가 발생할 수 있다.
	자바 프로그램에서는 이러한 에러가 발생하면 예외처리로 
	해당 에러에 대한 원인을 출력하고, 에러가 발생하더라도
	프로세스가 멈추지 않게 한다.
	
2. 웹 개발에서 위 내용과 함께 jsp 화면에서 에러에 대하여
	어떻게 표현하고, 처리할 것인지까지 포함한다.
	
3. 에러 처리 방식
	1) 각 에러가 발생한 페이지별로 에러 처리 페이지 설정
	2) 웹 서버 단위로 에러 내용에 따라 에러 처리 페이지 설정
		- web.xml에서 서버 단위로 에러 코드나 
		에러 객체를 선언하여 대체되는 화면을 로딩 처리한다.
		
# 각 화면별 에러 처리 페이지 구현 
1. 에러가 발생했을 떄 처리하는 페이지 설정
	<%@page isErrorPage = "true"%>
	위 옵션을 통해서 에러가 발생했을 때, 대체하는 페이지임을
	명시하면 exception 객체를 참조값에 의해 바로 사용할 수 있고,
...
# 서버 단위로 에러 페이지 지정 구현
0. 위와 같이 모든 페이지에 설정을 하지 않더라도 효과적으로
	에러 페이지를 대체할 수 있다.
...
2. http 코드 에러 설정
	...
	<error-page>
		...
		webapp기준으로 하위에 특정한 경로에 에러페이지를
		지정하여 처리
		
	2) 응답코드 종류
		200 : 요청 성공적으로 처리
		301 : 요청한 자원이 이동, 에러 정보에 이동 위치를 알려준 후
				다시 요청
		304 : 클라이언트가 임시 보관한 응답 결과와 다르지 않다.
		400 : 잘못된 요청
		401 : 인증 오류
		403 : 사용자 허가 모드 오류
		404 : 요청한 자원을 찾지 못한 경우
		405 : 요청한 method를 지원하지 않음
		500 : 서버 내부에서 오류가 발생
				프로그램적 오류(java 클래스 예외가 여기에 해당)
