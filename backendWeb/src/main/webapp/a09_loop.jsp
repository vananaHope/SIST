 ...
 --%>
    <div class="container mt-3">
		...
		<table class="table table-striped table-hover">
		    <tbody>
			   	<tr class="text-center">
			        <c:forEach var="x3" begin="3" end="51" step="3">
			        <td>${x3}</td>
			        </c:forEach>
			   	</tr>
		 	</tbody>
		...
		<%
		request.setAttribute("game",new String[]{"가위","바위","보"});
		%>
		<%--
		# <c:forEach>의 옵션(varStatus 처리 옵션)
		1. index : 루프 실행에서 현재 인덱스, count(루프실행횟수)
		2. begin : 시작 속성, end : 마지막 속성, step : 증/감 속성
		3. first : 현재 실행이 첫 번째 실행인 경우 true
		...
		<table class="table table-striped table-hover">
		    <tbody>
			   	<tr class="text-center">
			        <c:forEach var="game" varStatus="sts" items="${game}">
