...
public class A06_WrapperClass {
	...
	public static void main(String[] args) {
		...
		/*
		 # Wrapper 클래스
		 1. 기본 데이터 유형을 사용하다 보면 여러 형태로 변환이 필요하거나 
		 	기능적으로 처리해야 되는 경우가 많다. type이 기본 유형으로 되다보니 형변환이
		 	일어나지 않으면 이러한 기능에 대한 한계가 발생한다.
		 
		 2. 이 때, 자바 api에서는 기본 데이터 유형과 연결되는 객체 유형을 만들어
		 	필요에 따라 제공되는 메서드를 이용해서 여러가지 데이터 유형 변경, 치환, 기능 처리를 할 수 있게
...
//		numWrap01.XXXX() 를 통해 포함되는 여러 기능 메서드 지원
		...
		/*
		 ...
		 1. 기본 데이터 유형을 보다 간편하게 객체형으로 또는 기본 유형으로 변경 처리하는 기능이 필요해서
		...
		// ex) args에서 3000 true 40.2F
		// 		문자열로 입력받아서 각각 Wrapper 클래스에 할당하고 
		//		출력하세요....
		
		Integer w1 = Integer.valueOf(args[0]);
		Boolean b1 = Boolean.valueOf(args[1]);
		Float f1 = Float.valueOf(args[2]);
