...
public class Mart {
	...
	// 구매 갯수와 구매자 처리 메서드
	/*
	 # 1. 다단계에서 배열로 다중 객체를 선언할 떄는 배열자체가 초기에 크기를 설정하여 처리하여야 하는 부분이 있고,
	 		한 번 크기가 설정된 배열을 다시 객체로 생성하기 전까지는 변경이 불가하여 아래와 같이 
	 		추가할 때, 로직이 필요로 하다.
	 */
	public void buyProdPlan(String buyer, int cnt) {
		...
		System.out.println(buyer+"이(가) 물건 "+cnt+"종을 "+mname+"에서 구매하기로 했습니다.");
	}
	// 단위 구매 내용 처리
	public void buyProduct(Product prod) {
		...
		if( pArry != null) { // pArry.length nullpointer예외 방지 목적.
			...
			if(bIdx < cnt) {  // 2 < 3
				...
				System.out.println("구매할 종류 갯수("+cnt+") 더이상 구매불가합니다.");
			}
		}else {
			checkValid();
		}
