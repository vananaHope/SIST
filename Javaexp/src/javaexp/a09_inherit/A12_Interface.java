	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 1. 인터페이스 이해를 위한 이야기...
		 	...
		 	Flyway (인터페이스) : 날게됨
		 		껍데기(어깨에 정보를 입/출력하게 하는 처리 장치)
		 			중앙 처리장치에서 이 입/출력을 받아서 기기를 제어하게 처리
		 	
		 	Fly1호를 만들기 : 날개를 부착해서 붙여 놓은 인터페이스에 의해 정보를 처리하게 함... (실제객체)
		 	Fly2호.. Fly3g호...
		  ...
		  2. 인터페이스란?
		 	  ...
		 	  ex) Mz의 어깨위에 날개를 인식할 수 있는 장치가 인터페이스고
		 	  		이 인터페이스를 상속받아서 실제 날개1호, 날개2호 등이 부착되어 계속 업그레이드 가능하게 만듬...
		  ...
		  3. 인터페이스의 활용
		  	  ...
		  	  5) 인터페이스는 다중 상속이 가능하다.
		  	  		- extends 클래스 상속은 부모를 1개만 상속할 수 있지만
		  	  			인터페이스는 여러 개의 상위 객체를 implements할 수 있다.
		  	  			기능적으로 여러 개의 상위 인터페이스를 상속받아서 재정의 할 수 있다.
		  	  		
		  	  		# 상위의 부모를 하나밖에 상속하지 못하는 것을 확장시켜 기능적으로 여러 내용을 
		  	  			인터페이스에 의해 처리가능하게 해주는 기능
		...
		SwimmingWay sy01 = new Swimming01();
		sy01.swimming();
		
		SwimmingWay sy02 = new Swimming02();
		sy02.swimming();
...
//		addSkill로 SwimmingWay 할당하여, 
//		swimming()메서드를 통해, 해당 선수의 이름과 수영 하는 것을 처리하세요
...
class SwimmingPlayer {
	...
	public void swimming() {
		...
		if(swimmingway==null) {
			System.out.println(" 아직 수영 기술을 익히지 못했습니다.");
		}else {
...
class Swimming01 implements SwimmingWay {
	...
	@Override
	public void swimming() {
		System.out.println(" 우리동네 풀장을 "+SPORTS);
...
// 인터페이스를 상속받을 때는 implements 인터페이스로 선언한다.
