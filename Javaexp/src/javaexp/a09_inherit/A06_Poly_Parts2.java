...
class GraphicCard extends Part {
	...
	public void showInfo() {
		...
		super.showInfo();
		System.out.println("그래픽카드는 동적 화면을 처리합니다.");
	}
}
...
/*
 2단계 : 실무적 구조
 1:다관계 처리 구조
 컴퓨터 안에 여러 부품들을 장착 가능할 수 있도록 처리하였다.
 */
...
class ComputerParts{
	...
	private Part[] parts;
	private int idx;
	// 컴퓨터를 생성할 때, 제조사와 부품을 3개 장착할 수 있게 배열로 선언
	...
	// com.addPart(new Ssd());
	// com.addPart(new Ram());
	// Part part = new Cpu(); 형태로 다형성을 처리하는 상속구조가 되어 있기 때문이다.
	public void addPart(Part part) {
		// 3개 이상은 장착되지 못하게 처리 : 배열은 크기가 초기에 고정형으로 생성이 되기 때문이다.
		if(idx<3) {
			// parts[idx] = part;
			// parts[0] = new Cpu();
			// parts[1] = new Ram();
