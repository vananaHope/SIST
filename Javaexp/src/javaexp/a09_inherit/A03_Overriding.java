		 ...
		 3. 기본 형식
		    ...
		    class 하위1 extends 상위 {
		    	void 기능메서드() { 
		    		// 상위와 동일한 메서드명 정의
		    		# 주의 
		    		1. 메개변수의 유형과 갯수가 달라지면 오버로딩이 된다.
		    		   메서드명과 매개변수 유형과 갯수도 동일할 때를 말한다.
		    		2. 상위에서 선언한 내용과 하위마다 기능적 차별성이 있어야 한다.
		    		3. 접근제어자는 하위 객체에서 재정의하는 메서드의 접근 제어자는 상위와 같거나 더 넓어야 한다.    
...
class Worker {
	...
	public void working() {
		System.out.println(kind.equals("경찰관")?kind+"이 일을 합니다.":kind+"가 일을 합니다.");
	}
...
상위 공통된 Company을 통해서 상속받은 하위 객체들이 다양한 객체로 같은 메서드이지만 다른 기능을 처리
...
class Apple extends Company {
	...
	// 접근 제어자는 같거나 더 넓어져야 한다.
	public void earnMoney() {
		System.out.print("아이폰을 만들어서 ");
		super.earnMoney();
