...
public class A03_File {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 전송/출력을 할 수 있지만, 하드웨어적으로 저장하여 처리할 수도 있는데,
		 이렇게 하드웨어적으로 저장해주는 객체가 File 객체이다.
		 
		 2. 파일 객체의 처리하는 내용
		 	1) 일반 데이터를 물리적이고 장지간 저장하는 기능
		 	2) 장기간 저장한 데이터를 일반 데이터로 호출해서 사용
		 ...
		 3. 기능 메서드
		 	...
		 	String[] list() : 경로에 포함된 파일 또는 서브디렉토리 배열명 리턴
		...
		File f03 = new File(path+"a03_folder");
		File f04 = new File(path+"a04_folder");
		File f05 = new File(path+"a05_folder");
		File f06 = new File(path+"a06_folder");
		
		// ex) A05_DirFile.java를 만들고
		// javaexp.a13_io.a01_folder 밑에 a01~a09 폴더 생성하게 처리하세요
		...
		try {
			...
			// 경로를 만들 때 사용하는 메서드
			f03.mkdir(); // 경로를 만들 때는 필수 예외 필요 없음
			f04.mkdir();
			f05.mkdir();
			f06.mkdir();
		} catch (IOException e) {
