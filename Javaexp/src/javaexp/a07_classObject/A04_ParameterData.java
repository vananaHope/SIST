...
public class A04_ParameterData {
	...
	// void : 리턴값 없을 때 사용하는 메서드
	static void call() {
		System.out.println("함수0");
		System.out.println("매개변수로 없는 메서드");
	
	}
	// 하단에서 호출할 때, 어떤 메서드를 호출할 지 구분이 되지 않으므로 선언을 할 수 없다.
	// 변수명을 이름만으로 식별하지만, 함수형은 이름과 매개변수로 식별하여 호출할 수 있다.
//		static void call() {
//			System.out.println("함수0");
//			System.out.println("매개변수로 없는 메서드");
//		}
	...
	static void call03(String name, int age) {
		System.out.println("# 두 개의 매개변수로 온 함수3 #");
...
//		call("사과"); X ==> int num01 = "사과"; 
		call(25);
		call((int)25.7); // int num01 = (int)25.75;
		call02(25.7);
		call02(75); // double call02 = 75;
		/*
		 # 매개변수처리
		 1. 매개변수 생성자와 메서드에서 사용되는 데이터 전달 기능을 수행하는 변수이다.
		 2. 매개변수의 유형에 따라서 데이터를 전달할 수 있다.
		 	- 함수의 명과 데이터 유형이 맞아야 된다.
		 3. 매개변수는 순서, 타입, 갯수에 맞게 전달하여야 한다.
		 
		 # 생성자, 메서드의 overloading 주의) overriding과 구분
		 0. 궁극적인 원인..
		 	호출하는 곳에서 다르게 호출이 가능하느냐 여부에 따른 것이다.
		 	어느 함수를 호출하는 지 구분할 수 있느냐 여부
		 	변수는 이름으로 구분하지만, 함수는 이름과 매개변수로 구분한다.
		 	
		 1. 동일한 이름으로 정의 가능한 경우를 말한다.
		 	1) 매개변수의 갯수가 다르면 선언이 가능하다.
		 	2) 매개변수의 갯수가 같아도 데이터 타입이 다르면 선언이 가능하다.
		 	3) 매개변수의 갯수가 같고 타입이 같아도 순서가 다르면 선언이 가능하다.
		 */
		call();
		call(10);
		call(10.75);
