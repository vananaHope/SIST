...
public class A03_Constructor {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 2. 주요 기능
		 	...
		 	2) 필드를 초기화할 때, 사용된다.	
		 		...
		 		설정할 필요가 있을 때, 사용한다.
		 		ex) new Baby("귀염아기"), new Person("홍길동","25,3);
		 ...
		 3. 형태
		 	1) 기본 생성자
		 		...
		 		호출하여 사용한다. 즉, 컴파일러가 컴파일하면서 기본 생성자를 자동으로 추가한다.
		 		new 클래스명()
		 		
		 	2) 기본 생성자의 삭제 ***
		 		기본 생성자는 사용자 정의 생성자를 선언하는 순간, 사라진다.
		 		즉, 컴파일시 자동으로 추가하지 않는다.
		 	3) 추가 선언하는 생성자
		 ...
		 */
		 // 클래스명 기준으로 객체를 사용할 준비
		Person p01;
		p01 = new Person();
		
		ProductV0 prod01 = new ProductV0();
		// ex1) Computer 클래스를 선언하고 default생성자 호출
		// ex2) Mouse 클래스선언하고 매개변수가 없는 생성자 선언
		// 		마우스 객체 생성이라고 출력...호출
		// 둘 다 참조변수 com1, mou1을 호출하여 출력하세요
...
class Mouse {
	// 객체 속에 있는 field는 모두가 default로 초기화가 된 상태이기 때문에
	// 따로 default 데이터로 초기화할 필요가 없다.
	// 추가된 데이터로 할당만 하면 된다.
	String company;
	int price;
	Mouse() {
