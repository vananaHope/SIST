...
public class A12_StaticMember {
	...
	public static void main(String[] args) {
		...
		/*
		 # 상수
		 	객체마다 한 번 할당이 되면 변경되지 않는 수를 상수라고 한다.
		 	final 키워드 할당.
		 	1) 객체별 상수
		 ...
		 # 클래스로 객체를 생성해서 사용하다보니,
		   동일한 클래스를 통해서 나온 객체들이 공유하는 메모리의 필요성으로
		   생성된 것이 static 변수/상수이다.
		   
		   static은 객체 공유 메모리이기에 
		   변수로는 클래스명.static변수로 사용하고
		   메서드로는 클래스명.static메서드() 로 사용한다.
		   ex) Math.random() : api(내부 사용자 정의 객체) - Math객체를 공유하여 사용하는 공유필드 가진 데이터를
		   					   리턴하는 메서드이다
		   
		 # 객체가 공유하고 한 번 할당되면 변경할 수 없는 데이터를 설정하는 것을
		 	static final이라고 한다.
		 	클래스가 Math
		 	Math m = new Math();
		 	m.random()
		...
		// static 메서드는 객체 생성없이 호출 할 수 있는 메서드이기 때문에
		// 객체 생성이 필요한 일반 메서드를 포함할 수 없다.
		...
		Number01.no3 = 10;
		n.no1 = 8;
//		n.no2 = 9; // final이라는 키워드는 한 번 할당하면 변경할 수 없기에 에러가 발생한다.
		...
		// 객체가 공유하는 메모리를 사용하기 때문에 객체 공유라기 보다 
		// 클래스 변수로 사용된다.
		// 클래스명.변수명
		// BankAccount.totRes
		...
		System.out.println("신길동의 계좌: "+acc03.rest);
		System.out.println("홍길동의 계좌: "+acc01.rest);
		System.out.println("전체 계좌: "+BankAccount.totRes);
		
		System.out.println("static 메서드: "+BankAccount.savingMoney2(1000));
		System.out.println("static 메서드: "+BankAccount.savingMoney2(1000));
		
		System.out.println("acc03에서 공유 메모리 확인: "+BankAccount.totRes);
...
// 은행의 모든 계좌가 공유하는 정보(전체 계좌의 총계/계좌의 갯수)는 static이라는 키워드를 통해서
// 선언하면 효과적으로 처리할 수 있다.
class BankAccount {
	String name; // 계좌명	
	int rest; // 계좌별 총 잔액
...
//		rest++; 일반 변수는 에러 발생
	...
	// 3. 일반 메서드는 static 변수, static 메서드를 호출할 수 있다.
