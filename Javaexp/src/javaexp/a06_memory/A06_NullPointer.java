...
public class A06_NullPointer {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 2. 이 떄, null이라는 말 자체는 heap영역에 아직 할당하지 않았다는 의미를 
		 	가지고 있다.
		 3. 이 상황에 해당 객체가 가지고 있는 구성요소를 호출하면
		 ...
		 */
		// 선언 ==> 초기화(null,객체할당) ==> 사용
		// 1. 선언만 하고 초기화하지 않고는 사용불가(컴파일 에러)
		// 2. null로 초기화하고 
		//		1) 객체자체를 호출(실행 에러발생하지 않고) : null
		//		2) 객체의 구성요소 index, 속성 : 런타임 에러
		//		ArrayIndexOutOfBoundsException
		// 		NullPointerException
		// 3. 객체를 생성
		//		1) 해당 객체가 가지고 있는 범위 안에  배열/속성 : 호출
		//		2) 해당 객체가 가지고 있지 않은 범위의 배열index/속성 :
//					ArrayIndexOutOfBoundsException
//					NullPointerException
...
//		=> 객체는 초기화를 하여야 컴파일(기계어)로 바뀌어서 메모리에 올릴 수 있다.
		...
		// heap영역에 할당한 경우에 정상 처리
		 System.out.println(str02.charAt(0));
		 // 홍길동 : 기능 메서드가 지원하는 메모리 범위까지만 호출된다. : index 0~2
...
//		System.out.println(intArry[3]); => 에러발생
		
		// ex) 문자열로 물건명 3개를 할당하는 배열을 선언하고,
		//		각 index별로 호출하고, index범위가
		// 		초과할 때 나오는 에러를 확인하세요
		
		String[] prodName = {"사과","딸기","복숭아"};
		System.out.println(prodName);
//		[Ljava.lang.String;@58ceff1 데이터유형@16진수로 heap영역 주소값을 저장
		System.out.println(prodName[0]);
		System.out.println(prodName[1]);
		System.out.println(prodName[2]);
//		System.out.println(prodName[3]);  
//		=> ArrayIndexOutOfBoundsException 에러 발생 
//		   배열의 범위를 넘어섰기 때문에 에러 발생
