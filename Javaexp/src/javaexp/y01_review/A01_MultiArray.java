...
public class A01_MultiArray {
	...
	public static void main(String[] args) {
		...
		//		1) 외부 for문을 호차 처리 : 바깥 for문의 길이
		//		   trains.length
		for (int idx=0;idx<trains.length;idx++) {
			// 배열이 0부터 처리됨으로 1호차부터 나타내기 위해 
			// index번호 + 1로 처리함
			System.out.println(idx+1+"호차");
			// trains[idx] : 각 호차를 지칭
			// 각 호차 안에 좌석 배열 데이터가 들어가 있기에
			// 이것을 for문으로 처리한다.
			// trains[0].length : 1호차의 길이..
			// trains[0][0].length : 1호차의 첫 번째 좌석의 예약정보
			// trains[0][1].length : 1호차의 두 번째 좌석의 예약정보
			// trains[6][69].length : 7호차의 70번 째 좌석의 예약정보
