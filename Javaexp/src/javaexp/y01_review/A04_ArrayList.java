...
public class A04_ArrayList {
	...
	public static void main(String[] args) {
		...
		slist.add(new Student("20060203","김길동",90,2,18,70,90));
		slist.add(new Student("20060204","신길동",100,3,17,90,90));
...
/*
 ...
 1. 개념 : 데이터를 나열해서 처리하는 것은 배열이라는 개념부터 시작되었다.
 		  ...
 		  서현 구조로 하나씩 추가할 수 있는 구조를 말한다.
 		  class로 선언되어 객체가 만들어 질 때, 객체를 generic 형식으로 사용하면 보다
 		  효과적으로 객체형 list 데이터를 처리할 수 있다.
 2. 기본 사용
 	1) 생성: 
 ...
 3. 처리 순서(객체형 ArrayList)
 	1) 보통 2차원의 데이터(EXCEL데이터)를 한 개 row를 기준으로 
 		데이터 유형과 크기에 맞게 하나의 VO객체를 생성
 	2) class XXX{}로 선언되면, 생성자와 get/set 메서드 생성
 	3) 위 내용을 행단위 데이터를 객체 generic 타입으로 선언과 생성
 		생성
 		ArrayList<XXX> slist = new ArrayList<XXX>();
 	4) 그외 ArrayList 메서드를 이용하여 추가, 가져오기, for문 처리 등을 한다.
 		slist.add(new @@@@());
 		@@@@ s = slist.get(idx);
 		slist.size() 크기 가져오기..
 4. 응용 예제
