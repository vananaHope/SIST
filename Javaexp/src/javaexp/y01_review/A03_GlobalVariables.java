...
public class A03_GlobalVariables {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 1. 자바에서는 변수가 {}(중괄호 블럭)단위로 상대적으로 상위에 선언되어 있는 변수와 하위에 선언되어
		 있는 변수가 있는데, 상위에 선언되어 하위에 모든 중괄호 블럭에 영향을 미치는 변수를 전역변수,
		 해당 블럭의 매개변수나 선언된 것으로 상위에 해당 블럭에서만 사용되는 변수를 지역변수라고 한다.
...
//			System.out.println(cnt); 
//			for 매개변수 선언된 지역변수 X, 블럭 밖에서는 사용 못함
			int no = 1;
			// 초기값;반복조건;증감연산자
			for(no=5;no<=10;no++) {
		 ...
		 3. 활용 형태
		    ...
		    3) 클래스에서 선언된 전역변수인 필드는 this필드명 형태로 지역변수와 구분한다.
