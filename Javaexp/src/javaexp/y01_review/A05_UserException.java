...
/*
 ...
 3. 활용
 	1) 사용자 정의 예외는 일반적으로 api에서 지원되는 예외와 더불어 특정한 경우에
 	   예외를 던져 해당 정의된 예외 클래스에서 만들어지는 객체로 처리하는 경우를 말한다.
 	2) 사용자 정의 예외 처리 예시
 		- 아이디 패스워드에 입력 유효성 처리
 		- 회원 가입 아이디 입력 유효성 처리
 		- 특정한 입력값에 대한 유효성 처리(범위값을 넘길 때)
 		- 네트워크 서버 접속시, 접속 불가능할 때..
 4. 처리 순서
 ...
 5. 사용예제
 	...
 	- 메시지 : 입력한 아이디의 글자는 @@자 입니다. 
 	  ...
 	  기능내뇽 : getMessage()로 위 메시지 내용 재정의 처리
 	- 위 내용 기반 예외 클래스 정의  
 	- 해당 내용이 처리할 조건 및 프로세스 정리
 		Scanner로 입력시 글자가 8~16자가 아닐 때, 예외 던지기
