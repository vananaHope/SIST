...
1. jsp에서 사용가능한 새로운 스크립트 언어
	jsp 프로그램은 그동안 scriptlet, expression, declare 형태로
	사용했는 데 변수 설정과 scope에 따른 처리를 하다보면 예외를 처리해야
	하고, scriptlet의 복잡성으로 인해 많은 에러가 발생할 가능성이 있다.	
2. 이를 해결하기 위해, null 예외를 줄이고, scope에 따른 변수 설정을 효과적으로
	처리하는 언어를 jsp에서 지원하게 되었다.
3. el의 사용과 기본 코드
	1) el코드의 기본 형식
		el 코드는 scope에 의해 설정된 데이터를 가져올 때,
		기본의 getAttribute()로 가져오거나 요청값을 처리할 때,
		화면단에서 처리해야 하는 데이터를 간편하게 ${@@}형식으로
		처리 할 수 있다.
		- 기본 변수 ${변수명}
		- 객체형 데이터 ${변수명.property}	
			변수명.get프로퍼티명()과 동일
			주의) private 접근제어자인 속성이 아니고 getXXX()의
			get을 빼고 소문자로 치리된 데이터이다.
			${m.name} ==> <%=m.getName()%>와 동일	
4. el의 주요 기능
	1) jsp의 4자기 기본 객체의 범위에 따라 영역의 속성과 함께 변수로
		지정
	...
	el와 jstl을 이용하면 script(scriptlet, expression)을
	사용하지 않을 수 있다. 					
 --%>
 <%
 ...
 request.setAttribute("pname","바나나");
 request.setAttribute("price",5000);
...
<body>
    <div class="container mt-3">
    	...
    	<%--
    	# a02_el_session.jsp
    	<h2>이름:${name}</h2> request 범위라서 사라짐
    	<h2>나이:${age}</h2>   	
    	<%
