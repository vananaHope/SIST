...
2. 범위 내용
	...
	3) session영역 : 하나의 웹브라우저와 서버가 관련된 영역
...
4. 각 범위에 따른 처리
	...
	forward 처리했을 때, 다른 페이지 이더라도 사용할 수 있는 범위이다.
	...
	3) session
		...
		<location.href="B.jsp">					사용가능하다.
		response.sendRedirect("B.jsp");
		주의) 위 코드로는 request 범위로 설정한 데이터는
		사라진다.	
		
		#브라우저상 범위
		1. 브라우저를 닫지 않고, B.jsp의 주소를 복사해서 붙여도
			살아있다.
		2. 모든 브라우저를 닫고 B.jsp의 주소를 복사해서 붙여넣으면 
			session은 사라진 것을 볼 수 있다.
		...
		1. login ==> 인증되었을 DB id,pass외 이름 권한 포인트
		 	...
		 	페이지가 권한별로 접근할 때
				...
				Memeber mem = (Member)session.getAttribute("mem")
	...
	4) application
		서버가 살아 있는한 데이터를 지속적으로 저장하는 것을 말한다.			
 		위 page/request/session 범위를 포함해서 
