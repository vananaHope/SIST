...
3. 주요 객체 내용
	1) page : JSP 페이지에서 하나의 페이지 구현에 사용하는 자바 클래스의 인스턴스이다.
	...
	pageContext : 한 페이지 내에서만 
	request : 요청한 페이지까지 전달 
	session : 서버 vs 브라우저가 연결되어 있을 때까지
	application : 서버가 재기동하는 범위까지 
	
# out 기본 객체
1. jsp 페이지가 생성하는 모든 내용은 out기본 객체를 통해서 전송할 수 있다.
2. 복잡한 if-else문 사용시, out 기본 객체 사용하면 편리하다.
	1) scriptlet, expression의 복잡한 사용보다
		특정한 경우 out으로 활용할 때, 더 효율적인 경우가 많다.
3. 출력 메서드
 ...
 <%-- ex) 1~25까지 5x5 테이블을 out이용하여 출력하되,
  ...
  <table border width="30%">
  	<%for(int num=1;num<=25;num++) {
  		...
  		if(num%5==1) out.print("<tr>");
  		out.print("<td onclick=\"bg(this)\">"+num+"</td>");
  		if(num%5==0) out.print("</tr>");
  	
  	}%> 
  </table>
    <script>
  	function bg(ob) {
  		ob.style.background = "yellow";
  	}
  </script>
</body>
</html>