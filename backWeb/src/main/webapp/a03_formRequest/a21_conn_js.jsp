...
<%--
# front단과 backend단 연결 처리
1. 프로그램이 처리되는 흐름
	1) 요청(요청값) url로 프론트 ==> 서버단에 특정 자원(jsp) 요청값
		받아서 ==> java+html 코드 처리해서 html 만들어서
		==> 클라이언트에 html(html+css+js)를 전달 ==>
		html을 받아서 브라우저 프로그램이 렌더링(해석) 출력 
	
		# form 화면의 경우
		1. 초기 페이지 호출(html/jsp)
			url로 서버에 전달
		2. 초기 페이지에서 입력을 하고, 다시 서버호출
			요청값 + jsp를 호출
	...
	2) 특히, 화면에 DOM객체를 호출하여 이벤트 핸들러 처리하거나
		...
		처리하거나, 가장 하단에 script 선언해야 한다.
	

2. 단계별 처리
	1) backend에서 넘겨준 변수, js에서 처리
	2) 요청값을 넘겨야 JS코드를 만든 후, 처리
...
<script type="text/javascript">
	...
	// 다시 서버에 넘기는 방법??? form 객체 or query string 으로 전달
	// ex) a22_con_js.jsp
