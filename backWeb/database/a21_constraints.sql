 ...
 4. 무결성 제약조건의 종류
 	...
 	2) unique key : 테이블의 모든 행에서 고유한 값을 갖는 열 또는 열 조합을 지정해야 한다.
 		...
 		ps) null은 입력이 가능하고, 여러가지 null은 중복이 가능	
 			무조건 primary key을 통해서 not null + 구분자로 처리하는 것이 아니라 
 			구분을 하되 데이터가 확정되지 않은 경우는 null로 할당하게 할 때, 사용된다.					  
 	...
 	4) foreign key : 한 열과 참조된 테이블의 열 간의 외래 키 관계를 설정하고 시행
 		...
 		주의) 연관 테이블에 식별자와 비식별로 처리할 때, 사용된다.
...
--		가격 설정하여 sys.all_constraints 테이블 확인, 입력 처리하세요

CREATE TABLE product02(
	pid varchar2(50) CONSTRAINT product02_pid_pk PRIMARY KEY,
...
INSERT INTO product02 VALUES ('PID-1000','바나나',3000);
INSERT INTO product02 VALUES ('PID-1000',NULL,3000);

SELECT * FROM SYS.ALL_CONSTRAINTS
WHERE TABLE_NAME LIKE 'product02';

-- ex) sequence 예제 1100으로 시작하는 sequence을 만들어서 product02 테이블에 10개 데이터를 입력 처리하세요

CREATE sequence seq00
	START WITH 1100;

INSERT INTO product02 VALUES ('PID-'||seq00.nextval,'귤',1500);
