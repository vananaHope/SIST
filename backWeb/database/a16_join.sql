/*
 # 조인
 1. 하나의 sql명령문에 의해 여러 테이블에 저장된 데이터를 한 번에 조회할 수 있는 기능을 말한다.
 ...
 2. 위 기본 조인 구문 형식은 두 개의 테이블을 공통 컬럼 기준으로
 	inner 조인하는 것을 말한다.
 3. 		 	
...
-- emp테이블과 dept테이블은 deptno를 기준으로 연관관계가 설정되어 있다.
-- 이 때, 이 연관관계에 있는 컬럼을 조건으로 조인하는 것을 
...
-- 두 개의 테이블의 조인을 통해서 연관관계에 있는 컬럼의 내용을 
-- 출력할 수 있다.
-- ex) 조인 구문을 이용하여 부서명, 사원명, 부서위치, 급여를 출력하세요
...
--		확인하여 inner조인으로 처리하되
--		조건으로 salary는 5000이상, department_name은 Marketing이거나 Purchasing 데이터를
--		검색하여, 사원명, 급여, 부서명을 출력하세요

SELECT FIRST_NAME 사원명, salary 급여, d.department_name 부서명
