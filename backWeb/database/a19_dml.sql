/*
 ...
 2. 종류
 	...
 	3) delete : 삭제 처리 명령어
 3. 트랜잭션
 	 여러 개의 명령문을 하나의 논리적인 작업 단위로 처리하는 기능을 말한다.
 	 1) 트랜잭션 명령어 종류
 	 	commit : 정상 종료 확정
 	 	rollback : 비정상종료 복구
 	 ex) DB클라이언트가 접속해서 변경 했을 때, 본인 계정으로만 
 	 	 변경된 데이터가 보인다. rollback을 하면 다시 원복 할 수 있다. 
 	 	 (다른 계정으로 접근하면 변경되지 않는 데이터 보임) 	
 		 이 때, 변경 후, commit하는 순간 현재 클라이언트 뿐만 아니라 다른 계정으로 접속하더라도 변경되어 
 		 처리되고, 현재 계정을 rollback을 하더라도 복구가 되지 않는다.)
 ...
 3. 단일 행 입력
 	1) 기본 구문
 		...
 		- 특정 컬럼 입력
 			...
 			ex) insert ino emp(empno, ename, sal)
 						   value(1002,'마길동',3500.0);
 			ps) row단위로 입력되기에 지정되지 않는 컬럼은 null로 
 				처리된다.
 			
주의) 복사 테이블을 만들어서 처리, 기본 테이블은 변경하면 데이터를 
	 만들거나 변경할 수 없음. 			
 ...
 4. 다중행 입력
 	1) 전체 컬럼 입력
 		...
 		subquery의 조회되는 컬럼의 순서, 타입과 동일해야 한다.
...
INSERT INTO emp03 values(1002,'마길동','대리',7499,sysdate,5000,300,20);
...
INSERT INTO emp03(empno, ename, hiredate, deptno)
			values(1004,'오길동',to_date('2023/01/01','YYYY/MM/DD'),30);		
 ...
 */
 ...
 -- ex) dept03 테이블을 복사테이블로 만들고, ename, loc
--		의 컬럼 크기를 변경하고,
-- 		1) 전체 컬럼 입력
--		2) 지정 컬럼 입력(부서번호,부서명)
--		3) null 지정 컬럼 입력 처리
...
INSERT INTO dept03 values(50,'IT부서','서울');
INSERT INTO dept03(deptno,dname)
			values(60,'경영지원부서');
INSERT INTO dept03 values(70,'회계부서',null);
...
-- where 1=0 : false이면 데이터가 안들어가지는 구조만 복사된 테이블이 만들어진다.
...
FROM EMP e 
GROUP BY deptno;
-- ex) emp의 복사 테이블(emp08) 만들고, 부서별 최고급여자의 정보를 입력하세요
