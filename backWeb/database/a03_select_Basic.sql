/*
 ...
 2. 기본형식
 	 ...
 	 group by 그룹할 컬럼 지정 : 특정 컬럼 기준으로 합산 갯수,
 	 	최소/최대값 산정
 	 having : 그룹의 조건을 지정
...
-- ex) 사원의 이름은 @@@이고 급여와 보너스(급여의 15%)의 합산은 @@@원 입니다.

SELECT '사원의 이름은 '||ename ||'이고 급여와 보너스(급여15%)의 합산은 '|| (sal + sal*0.15) || ' 만원 입니다.' msg
FROM emp;
-- 문자열 안에 연산을 처리할 때는 ()로 우선연산 처리를 해서 활용한다.
