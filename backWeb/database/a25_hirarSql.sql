...
2. 게시물에 답글을 계층적으로 달아야 하는 경우, 직급에 있어서
상위 관리자와 하위 관리자를 구분하여 처리하는 경우, 메뉴 항목에 있어서
상위 메뉴와 하위 메뉴를 처리해야 하는 경우 등 여러가지 경우가
발생한다.

3. 이 때, 간혹 여러 테이블을 만들어서 계층적으로 처리하는 경우도
있지만, 계층적으로 하나의 테이블에 데이터를 처리하고, 오라클에서
사용하는 계층적 질의문을 처리하면 원하는 데이터가 계층적으로 출력되는
경우가 많다.

4. 계층적 질의문을 위한 테이블 구조
	1) 계층적 질의문 처리를 위해서는 테이블의 특정 컬럼의
	계층구조로 되어 있어야 한다.
	우리가 많이 아는 emp테이블의 empno, mgr이 전형적인 
	계층적 구조를 가진 컬럼구조로 empno, mgr은 사원번호와 관리자번호
	라는 관계가 다시 mgr은 empno에 연결관계가 있기 때문이다
  	결국, 최상위 관리자의 mgr번호에서 empno가 있고,
  	그 하위에 관리자나 사원들을 이 번호를 mgr의 번호로 설정되어 있기
  	때문이다.
...
5. 계층적 질의문의 기본 형식
	...
	4) 사용 코드
		...
		level : 계층 레벨을 가져온다. 위 공과 대학은 1level
			멀티미디어학과는 3level을 가져온다
		조건1 : 계층구조가 시작하는 조건을 말함
			테이블의 여러 계층이 있더라도 최상위에서 시작할 수도 있고
			중간 계층에서 시작할 수 있기 때문이다.
		조건2 : 계층 구조에서 상위와 하위를 연결하는 컬럼을 말함
...
ex) part01 테이블을 위 구조와 데이터로 만들고
계층적 sql로 level를 출력하세요
