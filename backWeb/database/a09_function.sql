...
/*
 ...
 # 실제 Dao단 처리시, pstmt에서는 pstmt.setString(1, "%"+emp.getFirst_name()+"%"); 
 */
...
/*
 ...
 2. lengthb(데이터/컬럼) : 입력되는 문자열의 바이트를 변환하는 함수
 		1) 영문은 1자와 byte는 같다
 		2) 한글을 1자가 3byte 이다.
 cf) dual : 오라클에서 지원되는 임시 가상 테이블로 데이터를 
 		간단하게 확인할 때 사용한다.		
 **/
...
SELECT ename, length(ename) "글자수", length(ename) "byte크기", 
		job, length(job) "글자수", lengthb(job) "byte크기"
FROM emp02;

-- ex) 사원 테이블에서 사원명과 직책의 글자 수를 표현하되,
--		직책의 글자개수가 6이상인 데이터만 출력하세요

SELECT length(ename) "사원명 글자수", length(job) "직책 글자수"
FROM EMP02 e 
WHERE length(job) >= 6;
...
/*
 ...
 # 기타 문자열 조작함수
 1. concat(문자열1, 문자열2) : 두 문자열을 결합 처리한다.
 	 문자열1 || 문자열2 동일
 	문자열1 || 문자열2 || 문자열3 ===> concat(concat(문자열1,문자열2),문자열3)
 2. substr(문자열데이터, 시작위치(1부터), 시작위치에서부터 개수)
 	문자열 데이터를 시작위치와 마지막 위치를 기준으로 절삭 처리하여 사용하는 것을 말한다.
...
SELECT concat(empno, ename) "사원명과 사원번호", substr(job,2,3)
FROM emp02;
