...
select 나 where 조건에 컬럼을 사용할 때, 공통 컬럼은 반드시 별칭.컬럼명으로 지정해서
호출하여야 하고 그 외는 별칭을 생략해도 무방하다.
select e.deptno, ename, dname,
from emp e, dept d
where e.deptno = d.deptno
사원 정보의 deptno와 부서정보의 deptno는 별칭으로 반드시 구분해서
사용해야 한다.
...
기본적으로 두 개이상 테이블을 결합하고 self join은 물리적으로 하나의 테이블이나
논리적으로 테이블을 alias로 호출하여 처리하기에 데이터 로딩의
관점에서는 두 개의 테이블이 조인되는 것과 같다.
ex)
from emp e, emp m
where e.mgr = m.empno
...
# subquery
1. select, where 조건의 컬럼과 매칭 시
	select (select max(sal) from emp) maxSal, 
			sal, ename
	from emp
	최대 급여를 각 row마다 보여주면서 사원의 급여와 이름 출력
...
9. outer join에 대한 설명으로 올바르지 않은 것은?
    ...
    D. outer join은 null값을 가진 행을 출력할 수 없다.
		두 테이블 중에 연관관계가 없는 테이블의 행을 null로 출력된다.
d

# outer join을 처리해야 할 경우
1. 사원 정보, 부서 정보에서 부서별로 소속된 사원을 출력하는 데,
	소속된 사원이 없더라도 부서 정보를 출력해야할 경우
	(inner join의 경우 소속된 사원이 없으면 부서 정보가 출력이 되지 않는다.)

2. 강좌와 수강신청학생과의 관계에서 수강신청을 하지 않는 강좌 정보도 
	출력해야 할 경우
